...
public class TestDistributedLock {
	...
	public static void main(String[] args) {
		...
		final ZkClientExt zkClientExt1 = new ZkClientExt("192.168.100.128:2181", 5000, 5000, new BytesPushThroughSerializer());
		final SimpleDistributedLockMutex mutex1 = new SimpleDistributedLockMutex(zkClientExt1, "/Mutex");
		
		final ZkClientExt zkClientExt2 = new ZkClientExt("192.168.100.128:2181", 5000, 5000, new BytesPushThroughSerializer());
		final SimpleDistributedLockMutex mutex2 = new SimpleDistributedLockMutex(zkClientExt2, "/Mutex");
		
		try {
			mutex1.acquire(); //mutex1获取锁
			System.out.println("Client1 locked");
			Thread client2Thd = new Thread(new Runnable() {
				
				public void run() {
					try {
                        System.out.println("client2 wait ...");
						mutex2.acquire();
